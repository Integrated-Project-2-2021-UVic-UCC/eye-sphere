    ...
    def create_connection(self,mobile,robot):
        for r in self.robots:
            if r.name==robot:
                ...
                send_msg('Connected successfully',mobile)
                r.connected=True
        ...
        if msg == '!DISCONNECT':                        #if the client dies, a !disconnect msg is recieved so we close and delete the client
            ...
            for obj in self.robots:
                if obj.name == rob:
                    ...
                    send_msg("1",self.clients_dic[reader])
                    self.create_connection(self.clients_dic[reader],rob)
                    ok=1
            
            if ok == 0:
                send_msg("0",self.clients_dic[reader])
...
def recv_data(client):
    ...
    final_data=client.get_data()[:data_size]
    client.save_data(client.get_data()[data_size:])
    client.connected_sock.client.sendall(header+final_data)
